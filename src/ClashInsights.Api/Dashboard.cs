...
public sealed record PlayerView(string Tag, string Name, int? TownHall, int? Trophies, int? Donations, int? Received, int? TrophyChange, DateTimeOffset ObservedAt, HistoryPoint[] History);
public sealed record MemberView(string Tag, string Name, string Role, int? Trophies, int? Donations);
public sealed record ClanView(string Tag, string Name, int? Level, int? Points, DateTimeOffset ObservedAt, MemberView[] Members, string[] Joined, string[] Left, bool RosterAvailable);
...
public static class Dashboard {
    public static DashboardView Build(List<Snapshot> snapshots, List<Attempt> attempts, TrackingOptions settings, string status) {
        var players = snapshots.Where(x => x.Kind == "player").GroupBy(x => x.Tag).Select(group => {
            ...
            return new PlayerView(latest.Tag, Text(p,"name"), Number(p,"townHallLevel"), Number(p,"trophies"), Number(p,"donations"), Number(p,"donationsReceived"), history.Length > 1 ? history[^1].Trophies-history[0].Trophies : null, latest.ObservedAt, history);
        ...
        var clans = snapshots.Where(x => x.Kind == "clan").GroupBy(x => x.Tag).Select(group => {
            ...
            var comparable = j.TryGetProperty("memberList", out _) && previousJson.TryGetProperty("memberList", out _);
            ...
            return new ClanView(latest.Tag, Text(j,"name"), Number(j,"clanLevel"), Number(j,"clanPoints"), latest.ObservedAt, members,
                ...
                previous.Where(m => members.All(p => p.Tag != m.Tag)).Select(m => m.Name).ToArray(), j.TryGetProperty("memberList", out _));
        }).ToArray();
    ...
    private static MemberView[] Members(JsonElement j) => j.TryGetProperty("memberList", out var members) && members.ValueKind == JsonValueKind.Array
        ? members.EnumerateArray().Select(m => new MemberView(Text(m,"tag"), Text(m,"name"), Text(m,"role"), Number(m,"trophies"), Number(m,"donations"))).ToArray() : [];
