...
## 3. Current phase
The source repository was made public on 2026-09-25 with MIT licensing, anonymized evidence, passing release CI and verified GitHub protections. `main` is the default branch and `develop` remains the integration branch. The September dependency refresh aligns Angular and Microsoft packages and adds coordinated lockfile maintenance; local checks and branch CI are green. Hosted security, public/private query boundaries, Auth0 plumbing, Sentry and container packaging are implemented. Local live collection was verified on 2026-09-18; the real clan identity is redacted. No hosted application has been provisioned or deployed. Current evidence is in test-plan.md and changes/006-dependency-refresh.md.
...
## 5. Remaining gates
1. Open and merge the green dependency-refresh branch through `develop`, close the superseded/incompatible Dependabot pull requests, then promote the batch to `main`. Local Docker-backed reruns remain blocked by a Docker Desktop 4.89.0 runtime-socket startup failure; Linux branch CI supplies the current backend, browser, Compose, and secret-scan evidence.
